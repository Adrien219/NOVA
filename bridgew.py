...
# Import compatible avec les versions récentes de paho-mqtt
try:
    # Version 2.0+
    from paho.mqtt.client import Client, MQTT_ERR_SUCCESS, MQTT_ERR_NO_CONN
    MQTT_V2 = True
except ImportError:
    # Version 1.x (fallback)
    from paho.mqtt.client import Client, MQTT_ERR_SUCCESS, MQTT_ERR_NO_CONN
    MQTT_V2 = False
...
class ArduinoBridge:
    ...
    - Compatible Windows et Raspberry Pi
    """
    ...
    def _init_mqtt(self):
        ...
        try:
            # Créer le client - compatible avec toutes les versions
            if MQTT_V2:
                self.client = Client(mqtt.CallbackAPIVersion.VERSION2, "SHOS_BRIDGE")
            else:
                self.client = Client("SHOS_BRIDGE")
            ...
            print(f"   MQTT Version 2: {MQTT_V2}")
            sys.exit(1)
    ...
    def _publish_mqtt(self, data):
        ...
        try:
            ...
            if result.rc == MQTT_ERR_SUCCESS or result.rc == 0:
